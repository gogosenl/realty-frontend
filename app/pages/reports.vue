  ...
  <div class="min-h-screen bg-gray-50">
    <nav class="bg-green-700 px-6 py-4 flex items-center gap-4 shadow-lg">
      <NuxtLink to="/" class="text-green-200 hover:text-white transition-colors"
        >← Geri</NuxtLink
      >
      <h1 class="text-xl font-semibold text-white">Finans Raporu</h1>
    </nav>
      ...
      <div v-if="summary" class="grid grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <p class="text-3xl font-bold text-blue-600 mt-1">
            {{ formatCurrency(summary.agencyRevenue) }}
          </p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <p class="text-sm text-gray-500">
            Toplam Emlak Danışman Ödemesi (%50)
          </p>
          <p class="text-3xl font-bold text-green-600 mt-1">
            {{ formatCurrency(summary.agentRevenue) }}
          </p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div v-else>
          ...
          >
            ...
            <div
              class="grid grid-cols-3 gap-4 mt-3 pt-3 border-t border-gray-50"
            >
              <div>
...
onMounted(async () => {
  ...
  try {
    ...
    summary.value = await $fetch(
      `${config.public.apiBase}/transactions/summary/financial`,
      {
        headers: getHeaders(),
      },
    );
