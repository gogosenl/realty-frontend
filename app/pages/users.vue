  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div
          v-if="successMsg"
          class="mt-3 text-green-600 text-sm bg-green-50 p-3 rounded-lg"
        >
          {{ successMsg }}
        </div>
        <div
          v-if="errorMsg"
          class="mt-3 text-red-500 text-sm bg-red-50 p-3 rounded-lg"
        >
          {{ errorMsg }}
        </div>
      </div>
...
onMounted(async () => {
  ...
  try {
    const [usersData, invitesData] = await Promise.all([
      ...
      $fetch(`${config.public.apiBase}/auth/invites`, {
        headers: getHeaders(),
      }),
    ]);
