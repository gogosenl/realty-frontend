<template>
  <div class="min-h-screen bg-gray-50">
    <!-- Navbar -->
    <nav class="bg-green-700 px-6 py-4 flex items-center justify-between shadow-lg">
      ...
      <div class="flex items-center gap-2">
        <ClientOnly>
          <template v-if="authStore.user?.role === 'admin'">
            <NuxtLink to="/agents" class="text-sm text-green-100 hover:text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">
              Emlak Danışmanları
            </NuxtLink>
            <NuxtLink to="/reports" class="text-sm text-green-100 hover:text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">
              Finans Raporu
            </NuxtLink>
            <NuxtLink to="/users" class="text-sm text-green-100 hover:text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">
              Kullanıcı Yönetimi
            </NuxtLink>
          </template>
        </ClientOnly>
        <NuxtLink to="/transactions/new" class="bg-white text-green-700 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-green-50 transition-colors">
          + Yeni İşlem
        </NuxtLink>
        <ClientOnly>
          <div class="flex items-center gap-2 ml-2 pl-2 border-l border-green-600">
            <div class="text-right">
              <p v-if="authStore.user" class="text-sm text-white font-medium">{{ authStore.user.name }}</p>
              <p v-if="authStore.user" class="text-xs text-green-200">{{ authStore.user.role === 'admin' ? 'Admin' : 'Emlak Danışmanı' }}</p>
            </div>
            <button
              @click="handleLogout"
    ...
    <div class="max-w-6xl mx-auto px-6 py-8">
      ...
      <ClientOnly>
        <!-- Agent Özet Kartı -->
        <div v-if="authStore.user?.role === 'agent'" class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
        ...
        <div v-if="authStore.user?.role === 'admin'" class="grid grid-cols-4 gap-4 mb-8">
          <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            ...
            <p class="text-3xl font-bold text-gray-800">{{ countByStage('agreement') }}</p>
          </div>
          <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-3">
            ...
            <p class="text-3xl font-bold text-gray-800">{{ countByStage('earnest_money') }}</p>
          </div>
          <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-3">
            ...
            <p class="text-3xl font-bold text-gray-800">{{ countByStage('title_deed') }}</p>
          </div>
          <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-3">
            ...
            <p class="text-3xl font-bold text-green-600">{{ countByStage('completed') }}</p>
          </div>
        </div>
      </ClientOnly>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        <div v-else-if="filteredTransactions.length === 0" class="p-12 text-center">
          ...
          <p class="text-gray-400">{{ selectedCity ? `${selectedCity} ilinde işlem yok` : 'Henüz işlem yok' }}</p>
        </div>
        ...
        <div v-else>
          ...
          >
            <div>
              <p class="font-medium text-gray-800 group-hover:text-green-700 transition-colors">{{ txn.propertyAddress }}</p>
              <p class="text-sm text-gray-500 mt-1">
                {{ txn.listingAgent?.name }} → {{ txn.sellingAgent?.name }}
              </p>
              <p v-if="txn.city" class="text-xs text-gray-400 mt-1">📍 {{ txn.city }}</p>
            </div>
            <div class="flex items-center gap-4">
              <p class="text-sm font-semibold text-gray-700">{{ formatCurrency(txn.totalServiceFee) }}</p>
...
const totalEarnings = computed(() => {
  return store.transactions
    ...
    .reduce((sum, t) => {
      ...
      let earning = 0
      if (listingEmail === userEmail) earning += t.commissionBreakdown.listingAgentAmount
      if (sellingEmail === userEmail) earning += t.commissionBreakdown.sellingAgentAmount
      return sum + earning
    }, 0)
})
