<template>
  <div class="min-h-screen bg-gray-50">
    <nav class="bg-white shadow px-6 py-4 flex items-center gap-4">
      <NuxtLink to="/agents" class="text-gray-500 hover:text-gray-700">← Geri</NuxtLink>
      <h1 class="text-xl font-semibold text-gray-800">Ajan Detayı</h1>
    </nav>
    ...
    <div v-else-if="agent" class="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-start justify-between">
          ...
          <span class="bg-blue-100 text-blue-700 text-sm px-4 py-1 rounded-full font-medium">Ajan</span>
        </div>

        <div class="grid grid-cols-3 gap-4 mt-6">
          <div class="bg-gray-50 rounded-lg p-4">
            <p class="text-xs text-gray-400">Toplam İşlem</p>
            <p class="text-2xl font-bold text-gray-800 mt-1">{{ agentTransactions.length }}</p>
          </div>
          <div class="bg-gray-50 rounded-lg p-4">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div v-if="agentTransactions.length === 0" class="p-8 text-center text-gray-400">
          Henüz işlem yok
        </div>
        ...
        <div v-else>
          ...
          >
            ...
            <div class="flex items-center gap-4">
              <div v-if="txn.stage === 'completed' && txn.commissionBreakdown" class="text-right">
                <p class="text-xs text-gray-400">Kazanç</p>
                <p class="text-sm font-semibold text-green-600">{{ formatCurrency(getAgentEarning(txn)) }}</p>
              </div>
...
onMounted(async () => {
  ...
  try {
    const [agentData, txnData] = await Promise.all([
      $fetch(`${config.public.apiBase}/agents/${route.params.id}`, { headers: getHeaders() }),
      $fetch(`${config.public.apiBase}/transactions`, { headers: getHeaders() }),
    ])
