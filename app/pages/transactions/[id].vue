<template>
  <div class="min-h-screen bg-gray-50">
    <nav class="bg-white shadow px-6 py-4 flex items-center justify-between">
      ...
      <div v-if="txn && txn.stage !== 'completed'" class="flex items-center gap-2">
        <button
    ...
    <div v-else-if="txn" class="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="flex items-center gap-2">
          <div v-for="(stage, index) in stages" :key="stage.key" class="flex items-center gap-2">
            <div :class="[
      ...
      <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg mx-4">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input v-model="editForm.propertyAddress" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Satış Fiyatı (₺)</label>
              <input v-model.number="editForm.salePrice" type="number" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Komisyon (₺)</label>
              <input v-model.number="editForm.totalServiceFee" type="number" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Notlar</label>
            <textarea v-model="editForm.notes" rows="3" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
          </div>
        </div>
        <div class="flex gap-3 mt-6">
          <button @click="handleEdit" :disabled="editLoading" class="flex-1 bg-blue-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
            {{ editLoading ? 'Kaydediliyor...' : 'Kaydet' }}
          </button>
          <button @click="showEditModal = false" class="flex-1 bg-gray-100 text-gray-600 py-2 rounded-lg text-sm font-medium hover:bg-gray-200">
            İptal
          </button>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <p class="text-sm text-gray-500 mb-6">Bu işlemi silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.</p>
        <div class="flex gap-3">
          <button @click="handleDelete" :disabled="deleteLoading" class="flex-1 bg-red-500 text-white py-2 rounded-lg text-sm font-medium hover:bg-red-600 disabled:opacity-50">
            {{ deleteLoading ? 'Siliniyor...' : 'Evet, Sil' }}
          </button>
          <button @click="showDeleteModal = false" class="flex-1 bg-gray-100 text-gray-600 py-2 rounded-lg text-sm font-medium hover:bg-gray-200">
            İptal
          </button>
        </div>
      </div>
